...
\end{itemize}
On the influence on magnetic field on the flowfield_temp
\begin{itemize}
...
\hfill
 ...
 \end{minipage}
 \caption{Time series of a north polar view on the radial magnetic field at the CMB. \textbf{Top row:} Case \textbf{P100} (see table \ref{tab:data}). The field behavior is highly time dependent but a preference for a structure with \textit{two} red patches of high intensity (azimuthal wave number $m=2$) is discernible. The imposed heat flux maxima lie at the top and the bottom, the minima on the right and left hand side. \textbf{Bottom row:} Case \textbf{U100}. Like in case P100 above, the field behavior is highly time-dependent but in contrast to the heat flux pattern case, the system rather promotes structures with \textit{one} flux patch. The color scale ranges from -0.5 (dark blue) to 2.5 (dark red). The \textbf{right end} shows the long-term averaged data. }
 ...
 \begin{tabular}{l | c| c |c |c| c| c| c| c |c}
  ...
  \hline
\rule{0pt}{4ex}\textbf{P100} & 2&  $10^{-4}$ & $440$ &$0$ & $8.6$ & \textbf{100\%} & 284.8 & 26.5 & 34\% \\ 
  \textbf{P69}   &2&  $10^{-4}$ & $352$ & $88$ & 10 & \textbf{69\%}	& 254.9 & 23.3 & 36\% \\       
  \textbf{P36}    &2&  $10^{-4}$& 220 & 220 & 12 & \textbf{36\%} & 209.9 & 16.3 & 44\% \\
  \textbf{P12}   &\textbf{2}&  $\bm{10^{-4}}$ & \textbf{88} & \textbf{352} & \textbf{14} & \textbf{12\%} & \textbf{170.3} & \textbf{7.4} & \textbf{57\%} \\
  \textbf{P0}    &2&  $10^{-4}$& 0 & 440 & 15.4 &\textbf{0\%} & 139.5 & 4.8 & 64\% \\
 \end{tabular}
\caption{Overview of the Parameters regarded in this section and the associated diagnostic quantities: Magnetic Reynolds number Re$_m$, Elsasser number $\Lambda$ and realtive strength of the magnetic dipole component $f_\textrm{dip}$, all defined in section \ref{sec:diagnostics}. The case P12, highlighted in bold, serves as showcase in section \ref{sec:dyn_d020}.}
\label{tab:data}
\end{table}
...
\subsubsection{Showcase: $\textrm{Ra}_\textrm{total} \bm {= 4.4 \times 10^6}$,  $\bm {\delta^*} = \bm{0.12}$}
\label{sec:dyn_d020}
The choice of the showcase with only a relatively mild portion of $\delta^* = 12$ \% thermal forcing will be justified in section \ref{sec:delta_dependence}. For the moment, the focus lies on the flux patch generation process. \\
...
\begin{figure}[H]
 ...
 \begin{minipage}{0.4\textwidth}
  ...
  \caption{Sketch of a north polar view on the vorticity distribution and the location of the magnetic flux patches for case P12, shown in figure \ref{fig:dyn_d20_vectorfield_Br}. Red patches mark cyclones, blue patches anticyclones and the black crosses indicate the position of the flux patches. The gray dashed line represents the intersection of the TC with the CMB. }
...
\begin{minipage}{0.48\textwidth}
 ...
 \caption{Surface flow field (scaled by the flow magnitude) and radial magnetic field component near the CMB at $r = 1.485$ for case P12. In the front hemisphere a dominant anticyclone with the center at $\sim 315^\circ$E and a weaker, bended cyclone with the center at $\sim 230^\circ$E are visible. Intense magnetic field correlates with the cyclonic region. View from a polar angle of $20^\circ$.  }
...
\subsubsection{Dependence on $\bm{\delta^*}$}
