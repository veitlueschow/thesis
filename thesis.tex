...
\label{sec:reldip}
The relative strength of the magnetic dipole component at the CMB is a measure that is accessible for dynamo simulations as well as for the geomagnetic field. Therefore it may serve as one indicators for the degree to which a dynamo model can be called Earth-like. The mantle, overlying the geodynamo, has a filtering effect on the magnetic field, so that only the first 12 spectral degrees of the geomagnetic field at the CMB are measurable at the Earth's surface. The relative dipole strength $f_\textrm{dip}$ is thus defined as the ratio of the rms axial dipole component to the total rms field up to the harmonic degree $l=12$. 
\begin{figure}[H]
...
\hfill
 ...
 \centering
   ...
   %\includegraphics[width = 0.98\textwidth]{results/reldip-crop.pdf}
 \caption{bla}
 \label{fig:reldip}
 \end{minipage}
\end{figure}
...
All results presented so far in section \ref{sec:dynamo} refer to the cases PFF and UFF that were all conducted with a total Rayleigh number $\textrm{Ra}_\textrm{total} = 440\times 10^4$ and an Ekman number Ek$=10^{-4}$ (see table \ref{tab:data}). In order to draw any conclusions relevant for the field of planetary cores, the robustness of the observations has to be checked, i.e. the study has to be extended to other parameters. Although it is impossible to reach a \textit{realistic} parameter regime, as in most problems linked to dynamos, a step in the right direction can be done. \\ \par \noindent
At first, the total Rayleigh number is varied while keeping $\delta^*=100\%$ fixed, so that the forcing is of purely thermal origin, i.e., $\textrm{Ra}_\textrm{total} = \textrm{Ra}_\textrm{\tiny T}$. The Ekman number is as above, Ek=$10^{-4}$. Figure \ref{fig:ra_ek4} shows the relative magnetic flux patch amplitude $\mathcal{P}$ (introduced in the preceding section) for different Rayleigh numbers $\textrm{Ra}_\textrm{\tiny T}$. A trend towards lower flux patch amplitudes for higher Rayleigh numbers is clearly discernible. Increasing the Rayleigh number means to increase the convective vigor and therefore the turbulent mixing. A more efficient mixing process tends to equilibrate large scale temperature gradients. Consequently, the influence of the boundary induced temperature variation is expected to be weakened and $\mathcal{P}$ to decrease. This is in line with the findings of \citet{hori2014ancient}, who reports the same behavior for internally heated dynamos. However, the question whether the influence of the $\mathcal{Y}_2^2$ boundary condition completely vanishes if the Rayleigh number is in a range realistic for the LOC, remains open. \\ \par \noindent
...
\begin{figure}[t]
 ...
 \caption{Relative flux patch amplitude for various Rayleigh numbers in a purely thermally driven case ($\delta^*=100\%$). The heat flux pattern amplitude is $q^*=2$, as above. The red point marks case PFF100 that was already discussed in section \ref{sec:effect_pattern}. }
 \label{fig:ra_ek4}
 \end{minipage}
 \hfill
  \begin{minipage}{0.47\textwidth}
 ...
 \caption{Elsasser number $\Lambda$ (black line, left ordinate) and relative dipole strength (red line, right ordinate) for different forcing ratios $\delta^*$. The total Rayleigh number is $\textrm{Ra}_\textrm{total} = 1000 \times 10^5$ and the Ekman number $\textrm{Ek}= 10^{-5}$ (critical Rayleigh numbers in table \ref{tab:crit}).}
 \label{fig:ek5}
 \end{minipage}
\end{figure}
