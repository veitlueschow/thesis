...
\begin{minipage}{0.48\textwidth}
 ...
 \caption*{(b)}
\end{minipage}
 \caption{(a) Elsasser number and (b) magnetic Reynolds number Re$_m$ (see section \ref{sec:diagnostics}) for different forcing ratios $\delta^*$ and the uniform case as well as for the case with $q^*=2$.}
   \label{fig:els_reym}
\end{figure} \noindent
The magnetic Reynolds number serves as a measure for a typical flow speed in a fluid and as it contains the rms-velocity, it is an indirect measure for the kinetic energy. Like the Elsasser number, $\textrm{Re}_m$ increases linearly with $\delta^*$ (see figure \ref{fig:els_reym}(b)). The above mentioned zonal flow, originating in a thermal wind, is a possible explanation for the increase in Re$_m$. The relative portion of kinetic energy $f_\textrm{tor}$ shown in figure \ref{fig:relTor}  

\begin{figure}[t]
 \centering
 \includegraphics[width = 0.5 \textwidth]{results/relTor-crop.pdf}
 \caption{Relative portion of toroidal kinetic energy $f_\textrm{tor}$ contained in the fluid (see section \ref{sec:diagnostics}).}
 \label{fig:relTor}
\end{figure}
