...
\begin{figure}[H]
 ...
 \caption{Sketch of the onset of convection for a rotating spherical shell. Convection columns form adjacent to the tangential cylinder (TC) \citep{busse1970thermal}. The columns can be visualized by different quantities. Here, they are sketched as contours of constant vorticity.}
...
\label{sec:dynamo}
The presence of a magnetic field has an effect on the flow field in a self-sustained dynamo via the Lorentz force term in equation 2.44. Nevertheless, it is widely assumed that column-like structures elongated parallel to the rotation axis can persist, even if the influence of the Lorentz force term in Equation 2.44 is comparable to or higher than the Coriolis force term \citep{christensen2011geodynamo}. This section presents magnetic field properties that are distinct to the $\mathcal{Y}_2^2$ boundary condition already applied in the previous, non magnetic case. Again, the influence of the heat flux pattern can best be observed in the mean flow field of the statistically stationary state. \\
...
\rule{0pt}{4ex}\textbf{PFF100} & 2&  $10^{-4}$ & $440$ &$0$ & $8.6$ & \textbf{100\%} & 284.8 & 26.5 & 34\% \\ 
 ...
 \end{tabular}
\caption{Overview of the Parameters regarded in this section and the associated diagnostic quantities: Magnetic Reynolds number Re$_m$, Elsasser number $\Lambda$ and realtive strength of the magnetic dipole component $f_\textrm{dip}$, all defined in section \ref{sec:diagnostics}. The case PFF12, highlighted in bold, serves as showcase in section \ref{sec:dyn_d020}.}
\label{tab:data}
\end{table}
This section aims to explain the formation mechanism for flux patches in the heat flux pattern case. Analogies to the mantle-locked structures presented in section \ref{sec:thermo-chemical} exist (the reader is reminded of the compositional field heterogeneities shown in figure \ref{fig:vort_chem}), but the presence of the Lorentz force clearly changes the dynamics of the system.

\subsection{The patch generation mechanism: A showcase}
\label{sec:dyn_d020}
The choice of the showcase PFF12 (see table \ref{tab:data}) with only a relatively mild portion of $\delta^* = 12$ \% thermal forcing will be justified in section \ref{sec:delta_dependence}. For the moment, the focus lies on the flux patch generation process. \\
...
\end{enumerate}
Each of the points mentioned is expected to have an effect on the behavior of the system (the corresponding literature will be mentioned in the following sections). The task is now to identify the individual effects of (a), (b) and (c) in the data and to separate them from each other as good as possible. 

\subsubsection{Boundary conditions: From Derichlet to Neumann}
\label{sec:ff_to_isochemical}
\citet{sakuraba2009generation} report the generation of a strong magnetic field in their dynamo model with a uniform heat flux imposed at the outer spherical boundary. The observation is remarkable because they used a comparatively low Ekman number ($\textrm{Ek} = 5 \times 10^{-7}$), for which the possibility of an efficient field production was questioned due to the extreme refinement of the azimuthal length scales when increasing the rotation rate \citep{sakuraba2009generation}. In contrast to the widely used fixed temperature condition, a fixed flux condition allows for lateral temperature gradients to develop at the CMB. \citet{gibbons2007convection} and \citet{sakuraba2009generation} therefore expect large scale zonal flows to develop in a thermal wind mechanism \citep{roberts2007theory}. As such flows are expected to be a necessary ingredient for an efficient dynamo process, the fixed flux condition indirectly has a positive effect on the magnetic field strength \citep{hori2012influence}.\\ \par
\begin{figure}[t]
...
\end{minipage}
 \caption{(a) Elsasser number, (b) magnetic Reynolds number Re$_m$ and (c) portion of toroidal kinetic energy $f_\textrm{tor}$ (see section \ref{sec:diagnostics}) for different forcing ratios $\delta^*$ for the uniform case UFF as well as for the case PFF with $q^*=2$. The data originates from the statistically stationary state.}
   \label{fig:els_reym}
\end{figure} \noindent
The Elsasser number $\Lambda$ compares the influence of the Lorentz force to that of the Coriolis force in the equation of momentum \citep{davidson2001introduction}. As the rotation rate is held constant here, $\Lambda$ serves as a measure for the strength of the magnetic field for the moment. Figure \ref{fig:els_reym}(a) shows $\Lambda$ for the different forcing ratios $\delta^*$, shown in table \ref{tab:data} and for different amplitudes of heterogeneity $q^*$.  For all values of $q^*$, the field strength increases roughly linearly with $\delta*$. According to the preceding discussion, this effect is attributed to the transition from a fixed composition BC at $\delta^* = 0$ to a fixed heat flux BC at $\delta^* = 1$. The increase of magnetic field strength can be observed for the uniform case UFF as well as for the case PFF. Yet, the heat flux pattern seems to additionally promote the magnetic field production. This effect will be further discussed when it comes to the influence of the heat flux \textit{pattern} in section \ref{sec:effect_pattern}\\ \par

The magnetic Reynolds number estimates relation between the advection and the diffusion of the magnetic field and as it contains the rms-velocity, it indirectly measures the kinetic energy. Like the Elsasser number, $\textrm{Re}_m$ increases linearly with $\delta^*$ (see figure \ref{fig:els_reym}(b)) for PFF and UFF.  \\
The zonal flow mentioned above, originating in a thermal wind, is a possible explanation for the increase in Re$_m$. Figure \ref{fig:els_reym}(c) that shows the relative portion of toroidal kinetic energy $f_\textrm{tor}$, supports this hypothesis: The increase of kinetic energy originates at least partly in an increase of its toroidal part that basically represents the azimuthal component of the velocity field. Presumably, the intensification of the zonal flow due to the transition to fixed flux conditions plays a crucial role for the increase of Re$_m$. \\
On the other hand, 


\subsubsection{The role of inertia: From Pr$_\textrm{\tiny C} = 3$ to Pr$_\textrm{\tiny T} = 0.3$}
\label{sec:effect_pr}

\subsubsection{The effect of the heat flux pattern}
