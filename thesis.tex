...
\subsection{Diagnostics (noch nicht vollständig)}
...
\begin{itemize}
 \item  Structure is of degree and order 2: Romanowicz,2003
\end{itemize}
...
\rule{0pt}{4ex}\textbf{PFF100} & 2&  $10^{-4}$ & $440$ &$0$ & $8.6$ & \textbf{100\%} & 284.8 & 26.5 & 62\% \\ 
 ...
 \end{tabular}
\caption{Overview of the Parameters regarded in this section and the associated diagnostic quantities: Magnetic Reynolds number Re$_m$, Elsasser number $\Lambda$ and realtive strength of the magnetic dipole component $f_\textrm{dip}$, all defined in section \ref{sec:diagnostics}. The case PFF12, highlighted in bold, serves as showcase in section \ref{sec:dyn_d020}. PFF stands for a fixed flux condition including the heat flux pattern, UFF for the uniform fixed flux BC.}
\label{tab:data}
\end{table}
...
A heterogeneous heat flux at the CMB (often referred to as \textit{core-mantle coupling} or \textit{mantle control of the geodynamo}) was explored by several authors and allowed some explanations for certain features of the geomagnetic field. \citet{bloxham1987thermal} make core-mantle interaction responsible for heterogeneities in the geomagnetic secular variation in the geomagnetic data of the last two centuries, \citet{glatzmaier1999role} and \citet{olson2010geodynamo} relate the geometry of the heat flux pattern to the uneven distribution of geomagnetic reversals in the paleomagnetic data of the Earth and \citet{olson2002time} and \citet{aubert2008thermochemical} find that the structure of the CMB heat flux is connected to persistent patches of high intensity magnetic flux in the geomagnetic data. \\ \par \noindent
A series of snapshots of the radial magnetic field component at the north pole shown in figure \ref{fig:fluxpatches_snapshots}, top row, gives a first insight into the behavior of the magnetic field under the influence of the heat flux pattern ($q^*=2$): Patches of intense magnetic field wander, merge and detach again in a highly time-dependent manner. Yet, the number of \textit{two} patches seems to be preferred by the system and therefore the azimuthal wave number $m=2$ dominates the magnetic energy spectrum (not shown). This is attributed to the the heterogeneous BC at the CMB that has the same azimuthal symmetry.\\
In contrast to that, figure \ref{fig:fluxpatches_snapshots}, bottom row, shows the uniform case ($q^*=0$) with the same Rayleigh number. The magnetic field strength is comparable but the magnetic energy in the $m=2$ mode does not dominate the spectrum. Instead, the number of \textit{one} flux patch seems to be preferred. The long-term averaged fields at the right end of figure \ref{fig:fluxpatches_snapshots} confirm the impression received from the snapshots.  \\ \par \noindent
This section presents magnetic field and flow field properties that are distinct to the $\mathcal{Y}_2^2$ boundary condition in the dynamo case. It will be shown, that some of the results found in the previous section got their analogies here, although the presence of the Lorentz force clearly changes the system behavior.\\
(ÜBERSICHT ÜBERS KAPITEL)
\begin{figure}[t]
...
\end{figure} \noindent
In the following, the formation mechanism of patches of intense magnetic field at the CMB (visible in figure \ref{fig:fluxpatches_snapshots}) will be discussed. Similarities with the mantle-locked structures presented in section \ref{sec:thermo-chemical} for the non magnetic case exist (the reader is reminded of the compositional field heterogeneities shown in figure \ref{fig:vort_chem}).\\ \par \noindent
Case PFF12 (see table \ref{tab:data}) with of portion of $\delta^* = 12$ \% thermal forcing was chosen as a showcase. For the moment, the focus lies on the flux patch generation process and not on the influence of the dual forcing mechanism.  \\ 
Although the uniform chemical forcing dominates in this case, the influence of the heat flux pattern is clearly visible in the surface projection of the radial magnetic field in figure \ref{fig:surface_Br_d20}. Patches of concentrated magnetic field lie at  $\sim$45$^\circ$ and $225^\circ$E in the northern and the southern hemisphere. They are each shifted $45^\circ$ west of the longitudes of maximum heat flux ($90^\circ$ and $270^\circ$, see figure \ref{fig:fluxsketch}). \\ \par \noindent
In section \ref{sec:attached_columns}, the flow convergence and divergence at the top of anticyclones and cyclones, respectively, controls the surface distribution of the chemical component (see figure \ref{fig:vort_chem}). In a comparable way, magnetic field can be collected (intensified) and expelled (weakened) by cyclonic and anticyclonic motion at the CMB in an advection process. A necessary condition for this to happen can be understood in the framework of \textit{Alfv\'{e}n's theorem} that states:\\ \par
...
Observation (b) is likely to be a consequence of the enlargement of the \textit{mean} azimuthal flow scale due to the emphasis of the azimuthal mode with the wavenumber 2, induced by the $\mathcal{Y}^2_2$ heat flux pattern. As previously discussed, flow scale refinement impedes the production of the magnetic field. The heterogeneous temperature boundary condition promotes large scales on which the dynamo process works more efficient. A more detailed discussion is beyond the scope of this study.
