...
\\
In order to implement the BC into the numerics, a (conductive) solution to \eqref{eq:laplace} has to be found. In spherical coordinates, it reads
\begin{equation}
...
\end{figure}
\\
The amplitude of heterogeneity, $Amp$, plays an important role for the heat flux pattern. Figure \ref{fig:equator_flux} 
\begin{figure}[H]
 \begin{minipage}[left]{0.6\textwidth}
  \includegraphics[width=1\textwidth]{sketches/eq_flux-trim.png}
\caption{Plot of the CMB - heat flux along the equator for amplitudes of heterogeneity $Amp_2^2 = 1$ and $Amp_2^2 = 2$. For $Amp > 1$, heat partially flows from the mantle to the core (blue region). } 
 \label{fig:equator_flux}
 \end{minipage}
 \hfill
  \begin{minipage}[right]{0.35\textwidth}
...
\begin{align}
\nonumber
 \frac{D \bm{ u}}{D  t} = &- \bm{  \nabla}  \pi - \frac{2}{\textrm{Ek}}\, \bm e_\textrm{\tiny{z}} \times \bm{  u} + \bm{  \nabla}^2 \bm{  u} + \frac{1}{\textrm{Ek}  \textrm{Pr}_\textrm{\tiny{m}}} \left( \bm{  \nabla} \times { \bm B} \right) \times { \bm B}  \\ 
 &+ (\textrm{Ra}_\textrm{\tiny {T}}  [\theta + t_2^2] + \textrm{Ra}_\textrm{\tiny {C}} { \zeta} )(1-a) {r} \, \bm e_\textrm{\tiny r}
\end{align}
