...
\end{equation}
where $\bm \nabla \cdot \underline{\tau} = -\bm \nabla p + \mu \bm \nabla^2 \bm u$ will be used as a reasonable simplification in the context of the Boussinesq approximation which will be discussed subsequently. This expression of $\bm \nabla \cdot \underline{\tau}$ is valid for Newtonian fluids and it is based on the assumption of a solenoidal velocity field ($\bm \nabla \cdot \bm u = 0$) and a homogeneous dynamic viscosity $\mu $ throughout the fluid. \\
The body forces $\bm f$ consist of buoyancy forces due to the gravity force $\bm g$ and the \textit{Lorentz force} $\bm f_l$ which will be discussed in section \ref{sec:lorentz}.
...
In the scope of core convection, a reduced form of the Maxwell equations (Pre-Maxwell equations) suffices (Zitat Davidson ???): 
...
The light component is released at the ICB as the inner core slowly crystallizes and serves as an additional source of buoyancy. In this model, the light component per unit mass, $C$, can only change by a flux $\bm q_\textrm{\tiny C}$through the surface of the fluid. According to section \ref{sec:internal_energy},
\begin{equation*}
