...
\begin{figure}[H]
 ...
 \end{minipage}
\caption{Time series of a north polar view on the radial magnetic field at the CMB for a purely thermal case with a \textbf{heat flux pattern} amplitude $q^*=2$ and $\textrm{Ra}_\textrm{\tiny T} = 4.4 \times 10^6$ (8.6 times supercritical). The field behavior is highly time dependent but a preference for a structure with \textit{two} red patches of high intensity (azimuthal wave number $m=2$) is discernible. The color scale ranges from -0.5 (dark blue) to 2.5 (dark red). The imposed heat flux maxima lie at the top and the bottom, the minima on the right and left hand side. The \textbf{right end} shows the long-term averaged data. }
\label{fig:fluxpatches1_snapshots}
\end{figure}
...
\begin{figure}[H]
 ...
 \end{minipage}
 \caption{Time series of a north polar view on the radial magnetic field at the CMB for a purely thermal case with \textbf{uniform} fixed flux conditions ($q^*=0$) and $\textrm{Ra}_\textrm{\tiny T}$ as in figure \ref{fig:fluxpatches1_snapshots}. Like above, the field behavior is highly time-dependent but in contrast to the heat flux pattern case, the system rather promotes structures with \textit{one} flux patch. The color scale is as in figure \ref{fig:fluxpatches1_snapshots}. The \textbf{right end} shows the long-term averaged data.}
 \label{fig:fluxpatches2_snapshots}
\end{figure}
...
\subsubsection{Showcase: $\textrm{Ra}_\textrm{total} \bm {= 4.4 \times 10^6}$,  $\bm {\delta^*} \bm= \bm{12}$\%}
\label{sec:dyn_d020}
