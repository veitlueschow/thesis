...
\begin{align}
  \int_{\mathcal{V}} \rho \frac{D \bm u}{D t} d^3r = \int_{\mathcal{V}} \bm f d^3r + \oint_{\partial \mathcal{V}} \bm t ds 
  - \int_{\mathcal{V}} \rho \left( 2\bm \Omega \times \bm u_R + \bm \Omega \times (\bm \Omega \times \bm r) \right) d^3r.
...
\end{figure}
The total density can be split into one spherical symmetric part $\bar{\rho}(r)$ that only depends on the radial level $r$ and the density variation $\delta \bar{\rho}(r,\vartheta,\varphi)$:
\begin{equation}
 \label{eq:density_split}
 \rho(r,\vartheta,\varphi) = \bar{\rho}(r) + \delta \bar{\rho}(r,\vartheta,\varphi).
\end{equation}
In case of a heat flux pattern proportional to the spherical harmonic $\mathcal{Y}_2^2$, the dependence of $\delta \bar{\rho}$ on $\varphi$ is $\pi$-periodic (see Figure \ref{fig:density_var}) so that it can be expressed by the product $\delta \bar{\rho}(r,\vartheta,\varphi) = \mathcal{C}(r,\vartheta) \cdot cos(2\varphi)$. $\mathcal{C}$ is only a function of $r$ and $\vartheta$. \\
...
\nonumber
&=- 4 \pi G \int\limits_0 ^r \int\limits_0^\pi \int\limits_0^{2\pi} sin(\vartheta) r'^2 [\bar{\rho}(r') + \delta \bar {\rho}(r',\vartheta,\varphi)] dr' d\vartheta d\varphi \\
\nonumber
&=-8\pi^2 G\int\limits_0^r \bar{\rho}(r') dr' -  4 \pi G \int\limits_0 ^r \int\limits_0^\pi sin(\vartheta) r'^2 \mathcal{C}(r',\vartheta) \left( \int\limits_0^{2\pi} cos(2\varphi) d\varphi \right) dr' d\vartheta \\
\nonumber
&= -8\pi^2 G\int\limits_0^r \bar{\rho}(r') dr'
\end{align}
Because the integration of $\delta \bar {\rho}(r',\vartheta,\varphi)$ over $\varphi$ drops out for every value of $r$ and $\vartheta$, $\bm g$ can be expressed by
\begin{equation}
 \bm g(\bm r) = -\frac{4 \pi G}{r^2}\int_0^r \bar{\rho}(r')r'^2 dr \bm e_r
\end{equation}
and it is worth noticing that $\bm g$ has the same form as in the spherical symmetric case.
...
A heat flux balance between the inner and the outer core boundary is imposed. The total influx at the ICB Q$_i$ equals the total outflux Q$_o$ at the CMB:
\begin{align}
...
\end{equation}
since there are no internal sources.\\
\begin{equation}
