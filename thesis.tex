...
\label{sec:dynamo}
The presence of a magnetic field has an effect on the flow field in a self-sustained dynamo via the Lorentz force term in equation 2.44. Nevertheless, it is widely assumed that column-like structures elongated parallel to the rotation axis can persist, even if the influence of the Lorentz force term in Equation 2.44 is comparable to or higher than the Coriolis force term \citep{christensen2011geodynamo}. \\
This section presents magnetic field properties that are distinct to the $\mathcal{Y}_2^2$ boundary condition already applied in the previous, non magnetic case. Again, the influence of the heat flux pattern can best be observed in the mean flow field of the statistically stationary state. \\
The supercriticalities in this section are defined according to the critical Rayleigh numbers for the non magnetic case, described in section \ref{sec:thermal/chemical}.
\subsection{Mantle-locked magnetic flux patches}
\label{sec:flux_patches}
The series of snapshots of the radial magnetic field component at the north pole shown in figure \ref{fig:fluxpatches_snapshots}, top row, gives a first insight into the magnetic field behavior under the influence of the heat flux pattern ($q^*=2$): Patches of intense magnetic field wander, merge and detach again in a highly time-dependent manner. Yet, the number of \textit{two} patches seems to be preferred by the system and therefore the azimuthal wave number $m=2$ dominates the magnetic energy spectrum (not shown). This is attributed to the the heterogeneous BC at the CMB that has the same azimuthal symmetry.\\ 
In contrast to that, figure \ref{fig:fluxpatches_snapshots}, bottom row, shows the uniform case ($q^*=0$) with the same Rayleigh number. The magnetic field strength is comparable but the magnetic energy in the $m=2$ mode does not dominate the spectrum. Instead the number of \textit{one} flux patch seems to be preferred. The long-term averaged fields at the right end of figure \ref{fig:fluxpatches_snapshots} confirm the impression that can be gained by the snapshots.  \\ 
...
\label{tab:data}
\end{table}
This section aims to explain the formation mechanism of intense magnetic patches in the heat flux pattern case. Analogies to the mantle-locked structures presented in section \ref{sec:thermo-chemical} for the non magnetic case exist (the reader is reminded of the compositional field heterogeneities shown in figure \ref{fig:vort_chem}), but the presence of the Lorentz force clearly changes the dynamics of the system.
\subsection{The patch generation mechanism: A showcase}
\label{sec:dyn_d020}
The choice of the showcase PFF12 (see table \ref{tab:data}) with only a relatively mild portion of $\delta^* = 12$ \% thermal forcing will be justified in section \ref{sec:effect_pattern}. For the moment, the focus lies on the flux patch generation process. \\
...
\subsection{Relative dipole contribution}
\begin{figure}[t]
 \centering
 \includegraphics[width = 0.4\textwidth]{results/rel-crop.pdf}
 \caption{bla}
